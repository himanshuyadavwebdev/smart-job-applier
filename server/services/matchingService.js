function calculateSkillOverlap(jobSkills, userSkills) {
...
function calculateSalaryFit(job, preferences) {
...
function calculateExperienceMatch(job, resume) {
  ...
  if (titleLower.includes('junior') || titleLower.includes('entry') || descLower.includes('0-2 years')) {
    jobLevel = 1;
  } else if (titleLower.includes('senior') || descLower.includes('5+ years') || descLower.includes('senior')) {
    jobLevel = 3;
...
function calculateLocationMatch(job, preferences) {
...
export function rankJobs(jobs, resume, preferences) {
  const ranked = jobs.map((job) => {
    ...
    const salaryScore = calculateSalaryFit(jobObj, preferences);
    const experienceScore = calculateExperienceMatch(jobObj, resume);
    const locationScore = calculateLocationMatch(jobObj, preferences);
  ...
  return ranked.sort((a, b) => b.matchScore - a.matchScore).slice(0, 100);
}
